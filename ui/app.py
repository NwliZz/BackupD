...
disk = data.get("disk", {})
total = disk.get("total_bytes", 0) or 0
used = disk.get("used_bytes", 0) or 0
free = disk.get("free_bytes", 0) or 0
pct = (used / total) if total else 0.0

st.markdown("---")
st.subheader("Storage")
st.progress(min(max(pct, 0.0), 1.0))
st.caption(f"Used: {hbytes(used)} • Free: {hbytes(free)} • Total: {hbytes(total)} • ({pct*100:.1f}%)")
